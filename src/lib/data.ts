...
export async function fetchPaginatedVocabInstantFeedback(items: number, page: number) {
    ...
    try {
      const data = await sql<VocabItem>`SELECT * FROM vocabularycards LIMIT ${pageItems} OFFSET ${offset}`;
      return data.rows;
    } catch (error) {
      console.error("Database Error:", error);
      throw new Error("Failed to fetch paginated vocabulary data for the instant feedback form.");
    }
  }
